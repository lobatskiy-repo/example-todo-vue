<template>
  <div>
    <!-- Modal -->
    <div class="modal fade" id="genericPopup" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          ...
          <div class="modal-body">
            <!-- <div class="content-wrapper">
              <div class="task-wrapper">
                ...
                <input type="text" class="form-control" :value="taskDetail.title" v-if="showEditTodoTitleField" @blur="saveTodoTitle" />
                <div class="task-actions">
                  <i class="fa fa-edit" aria-hidden="true" @click="editTodoTitle" v-if="showTitleBtn"></i>
                </div>
              </div>
             <div class="task-desc task-desc mt-3 text-right">
                <div class="task-description text-left small" v-if="showDescription">{{ taskDetail.description }}</div>
                <p class="text-primary add-link" @click="editTodoDescription" v-if="showDescriptionBtn">{{ taskDetail.description ? 'Edit ' : 'Add
                '}}description</p>
                <textarea v-if="showAddDecriptionField" class="form-control" rows="3" placeholder="Write a dewscription text here ..."
                  @blur="saveTodoDescription" :value="taskDetail.description"></textarea>
              </div>  
            </div> -->
            <div class="task-features">
              <div class="nav-wrapper">
                <ul class="nav nav-pills nav-fill flex-column flex-md-row" id="tabs-icons-text" role="tablist">
                  <li class="nav-item">
                    <a class="nav-link mb-sm-3 mb-md-0" id="tabs-icons-text-2-tab" data-toggle="tab" href="#tabs-icons-text-2" role="tab"
              ...
              <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade task-priority" id="tabs-icons-text-2" role="tabpanel" aria-labelledby="tabs-icons-text-2-tab">
                  <div class="card shadow">
                    <div class="card-body">
                      <p class="description">Set priority of your task</p>
                      <div class="custom-control custom-radio mb-2">
                        <input name="custom-radio-1" class="custom-control-input" id="customRadio1" type="radio" v-model="taskDetail.priority"
                          value="High" />
                      ...
                      <div class="custom-control custom-radio mb-2">
                        <input name="custom-radio-1" class="custom-control-input" id="customRadio2" type="radio" v-model="taskDetail.priority"
                          value="Medium" />
                      ...
                      <div class="custom-control custom-radio mb-2">
                        <input name="custom-radio-1" class="custom-control-input" id="customRadio3" type="radio" v-model="taskDetail.priority"
                          value="Low" />
                      ...
                      <div class="custom-control custom-radio mb-2">
                        <input name="custom-radio-1" class="custom-control-input" id="customRadio4" type="radio" v-model="taskDetail.priority"
                          value="None" />
                ...
                <div class="tab-pane fade task-tags" id="tabs-icons-text-3" role="tabpanel" aria-labelledby="tabs-icons-text-3-tab">
                  <div class="card shadow">
                    <div class="card-body">
                      <div class="new-tag">
                        <div class="row">
                          <div class="col-md-12">
                            <div class="form-group">
                              <input type="text" class="form-control form-control-alternative" id="exampleFormControlInput1" placeholder="Enter tag"
                                autocomplete="off" v-model="searchText" v-on:keydown.enter="createNewTag(searchText)" />
                            </div>
                          </div>
                        </div>
                      </div>
                      <ul class="tags-wrapper">
                        <!-- <li class="tag-item" v-for="(tag, key) in searchTag" :key="key">
                          <div class="color-option-icon">
                            <i class="fa fa-ellipsis-v" id="tagColorDropdoen1" aria-hidden="true" data-toggle="dropdown" aria-haspopup="true"
                              aria-expanded="false"></i>
                            <div class="dropdown-menu" aria-labelledby="tagColorDropdoen1">
                              <div class="group-color-dialog">
                                <a class="group-color-item" v-for="(color, index) in colorPalete" :key="index" :style="{ background: color }"
                                  @click="setTagColor(key, $event)"></a>
                              </div>
                            </div>
                          </div>
                          <div class="tag-icon">
                          ...
                          <div class="action">
                            <div class="custom-control custom-checkbox mb-3">
                              <input class="custom-control-input" :id="key" type="checkbox" v-model="taskDetail.tags" :value="tag" />
                        ...
                        <!-- <li v-if="searchTag.length <= 0" class="tag-item mark" @click="createNewTag(searchText)">Create "{{ searchText }}"</li> -->
                      </ul>
                    </div>
                  </div>
                </div>
