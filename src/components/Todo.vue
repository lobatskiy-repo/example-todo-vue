      ...
      <main class="container card shadow shadow-lg--hover mt-3" id="todolist">
        <div class="row mb-3">
          ...
          <div class="col-6 text-right">
            <div class="user-icon">
              <div class="dropdown">
                <img :src="getUserData.profileImage ? getUserData.profileImage : 'dist/assets/img/user-avatar.png'" id="dropdownMenuButton"
                  data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="dropdown-toggle img-fluid" />
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                  <a class="dropdown-item" href="#" v-if="userLoggedIn" @click="googleLogout">Logout</a>
                  <a class="dropdown-item" href="#" @click="toggleFullScreen">{{ isFullScreen ? 'Exit Full Screen' : 'Full Screen' }}</a>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="row">
          ...
          <div class="col-md-12 mt-3">
            <div class="form-group">
              <input type="text" name="newTodo" class="add-todo-field form-control" placeholder="Enter New To-Do"
                v-on:keydown.enter="addnewTodo($event)" autocomplete="off" v-model.trim="newTodoText" />
        ...
        <ul id="todo-list">
          <VuePerfectScrollbar class="scroll-area">
            <draggable handle=".handle-wrapper" ghost-class="ghost" :list="getTodos">
              <transition-group>
                <li class="todo-item" :class="todo.completed ? 'done' : 'undone'" v-for="(todo, key) in getTodos" :key="key"
                  ...
                  <div class="todo-tags">
                    <i class="fa fa-tag" aria-hidden="true" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></i>
                    ...
                    <div class="dropdown-menu" v-if="todo.tags.length > 0">
                      ...
                      <span class="badge badge-pill badge-info" :style="{ background: tag.color, color: '#fff' }" v-for="(tag, key) in todo.tags"
                        :key="key">
                        <!-- <i class="fa fa-tag" aria-hidden="true"></i> -->
                        {{ tag.name }}
                      </span>
                  ...
                  <div class="actions">
                    <button type="button" class="btn-picto" @click.stop="completeTodo(key)" :aria-label="todo.completed ? 'Undone' : 'Done'"
                    ...
                    <button @click.stop="removeTodo(key)" type="button" aria-label="Delete" title="Delete" class="btn-picto">
